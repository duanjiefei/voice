...
import com.github.duanjiefei.lib_audio.view.adapter.MusicListAdapter;
...
public class MusicListDialog extends BottomSheetDialog {
    ...
    private void updatePlayModeView() {
        switch (mPlayMode) {
            case LOOP:
                mTipView.setImageResource(R.drawable.loop);
                mPlayModeView.setText("列表循环");
                break;
            case RANDOM:
                mTipView.setImageResource(R.drawable.random);
                mPlayModeView.setText("随机播放");
                break;
            case REPEAT:
                mTipView.setImageResource(R.drawable.once);
